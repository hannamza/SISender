#include "stdafx.h"
#include "EventSend.h"
#include "Server.h"		//Server::Instance()->ProcessGFSProtocolUnSolicitedEvent(pData); 를 위해 헤더 추가
...
{
	if (!g_bThread) {
		Log::Trace("EventSend Process Started..");
		g_bThread = true;
...
void CEventSend::StopSendEventThread()
{
	...
	if (!m_pThread)
	{
		Log::Trace("EventSend Process Stopped..");
		return;
	}
	m_pThread->join();
...
void CEventSend::SendThreadLoop()
{
	...
	while (g_bThread)
	{
		...
		Server::Instance()->ProcessGFSProtocolUnSolicitedEvent(pData);
