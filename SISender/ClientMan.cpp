...
void ClientMan::Send(BYTE* buffer, int nSize)
{
	EnterCriticalSection(&m_CSForClients);

	for (int i = 0; i != static_cast<int>(m_listClient.size()); ++i)
	{
		m_listClient[i]->PostSend((char*)buffer, nSize);
	}

	LeaveCriticalSection(&m_CSForClients);
}

Client::State ClientMan::GetClientState()
{
	Client::State state = Client::CLOSED;
	...
	{
		TRACE("$$$$$$> 2222\n");
		if (m_listClient[i]->GetState() == Client::CONNECTED)
		{
			TRACE("$$$$$$> 3333\n");
			LeaveCriticalSection(&m_CSForClients);
			TRACE("$$$$$$> 4444\n");
