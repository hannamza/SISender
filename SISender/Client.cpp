		...
		if (WSARecv(m_Socket, &recvBufferDescriptor, 1, &numberOfBytes, &recvFlags, &event->overlapped, NULL) == SOCKET_ERROR)
		{
			...
			else
			{
				...
				if (m_State == CREATED)
				{
					...
					{
					case KOCOM:		//KOCOM은 Bind 필요
					{
						CClientInterface::Instance()->KOCOMProcessRequestBind(CCommonState::Instance()->m_szID, CCommonState::Instance()->m_szPass);
						break;
					}
					default:
						break;
