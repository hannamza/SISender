...
void CClientInterface::GFSProcessRequestLogin(CHAR* szID, CHAR* szPW)
{
	GFSProtocolLoginRequest login;
	strcpy_s(login.szID, szID);
	strcpy_s(login.szPW, szPW);

	Log::Trace("ID : %s, PW : %s GFS Login Request", szID, szPW);

// 	login.cmd = htonl(login.cmd);
// 	login.size = htonl(login.size);

	Send((BYTE*)&login, sizeof(GFSProtocolLoginRequest));
}

void CClientInterface::GFSProcessRequestKeepAlive()
{
	GFSProtocolKeepAliveRequest keepAlive;
	Send((BYTE*)&keepAlive, sizeof(GFSProtocolKeepAliveRequest));
}