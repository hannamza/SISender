...
namespace
{
	...
	/* static */ void IOEvent::Destroy()
	{
		IOEventPool::purge_memory();
	}
	...
	void PrintConnectionInfo(SOCKET socket)
	{
		...
		Log::Trace("Connection from ip[%s], port[%d] to ip[%s], port[%d] succeeded.", clientIP.c_str(), clientPort, serverIP.c_str(), serverPort);
	}
}
...
bool Client::PostConnect(const char* ip, short port)
{
	...
	struct addrinfo* info = m_infoList;
		...
		if (WSARecv(m_Socket, &recvBufferDescriptor, 1, &numberOfBytes, &recvFlags, &event->overlapped, NULL) == SOCKET_ERROR)
		{
			...
			else
			{
				...
				if (m_State == CREATED)
				{
					...
					//Login Request
					CClientInterface::Instance()->GFSProcessRequestLogin(CCommonState::Instance()->m_szID, CCommonState::Instance()->m_szPass);
				}
			}
		}
		else
...
// 	case SI_TEST:
// 	{
// 		WorkerProcessRecvPacket(m_recvBuffer, dwNumberOfBytesTransfered);
// 		break;
// 	}
// 	case KOCOM:
// 	{
// 		KocomWorkerProcessRecvPacket(m_recvBuffer, dwNumberOfBytesTransfered);
// 		break;
// 	}
// 	default:
// 		break;
// 	}
	...
	GFSWorkerProcessRecvPacket(m_recvBuffer, dwNumberOfBytesTransfered);
...
{
	//trace0("OnSend() : %d", dwNumberOfBytesTransfered);
}
